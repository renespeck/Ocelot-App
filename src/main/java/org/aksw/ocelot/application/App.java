...
public class App {
  ...
  public static Map<String, SimpleEntry<String, String>> uriToDomainRange = new HashMap<>();

  public static List<String> predicates =
      Arrays.asList("almaMater", "bandMember", "birthPlace", "broadcastArea", "child", "deathPlace",
          "department", "director", "district", "employer", "formerBandMember", "ground",
          "headquarter", "locatedInArea", "manager", "neighboringMunicipality", "partner",
          "regionServed", "relative", "riverMouth", "sisterStation", "spouse", "subsidiary");

  public ApplicationUtil applicationUtil;

  public App() {
    Const.RELATION_DOMAIN_PLACEHOLDER = "?D";
    Const.RELATION_RANGE_PLACEHOLDER = "?R";
    Const.TMP_FOLDER = Resources.getResource("tmp").getPath();
  ...
  public SimpleEntry<String, String> replace(final SimpleEntry<String, String> entry) {
    return new SimpleEntry<>(//
        entry.getKey()//
            .replaceAll(Pattern.quote("dbo:Organisation"), Application.O) //
            .replaceAll(Pattern.quote("dbo:Place"), Application.L) //
            .replaceAll(Pattern.quote("dbo:Person"), Application.P), //
        entry.getValue()//
            .replaceAll(Pattern.quote("dbo:Organisation"), Application.O)//
            .replaceAll(Pattern.quote("dbo:Place"), Application.L)//
            .replaceAll(Pattern.quote("dbo:Person"), Application.P)//
  ...
  public Set<String> run(final String sentence, final String domain, final String range,
    ...
    for (final String predicate : predicates) {
      ...
      final SimpleEntry<String, String> dr =
          uriToDomainRange.get("http://dbpedia.org/ontology/" + predicate);
  ...
  public Map<ColoredDirectedGraph, Set<ColoredDirectedGraph>> trees(String predicate) {
    final Map<ColoredDirectedGraph, Set<ColoredDirectedGraph>> generalizedTrees;
    generalizedTrees = applicationUtil.loadGeneralizedTrees(predicate);
